...
import PropertyPostForm from "./PropertyPostForm/PropertyPostForm";
import PropertyUpdateForm from "./PropertyUpdateForm/PropertyUpdateForm";
...
export default function OptionsPage() {
  ...
  async function getData() {
    try {
      setLoading(true);
      let response = await callApi({ endpoint: `/api/propertys` });
      if (response) {
    ...
    <React.Fragment>
      ...
      {modalInfo.type === "POST" && (
        <Modal
          ...
          title="Property Ekleme Ekranı"
          onCancel={hideModal}
          footer={null}
        >
          <PropertyPostForm handleClose={hideModal} />
        </Modal>
      )}
      ...
      {modalInfo.type === "UPDATE" && (
        <Modal
          visible={modalInfo.visible}
          title="Property Güncelleme Ekranı"
          onCancel={hideModal}
          footer={null}
        >
          <PropertyUpdateForm
      ...
      {modalInfo.type === "ITEMS" && (
        <Modal
          visible={modalInfo.visible}
          title="PropertyItems Ekranı"
          onCancel={hideModal}
          footer={null}
          width={1000}
        >
          <PropertyItems handleClose={hideModal} data={modalInfo.selected} />
        </Modal>
      )}
    </React.Fragment>
  );
