  ...
  return (
    <React.Fragment>
      ...
      <Button type="primary" style={{ marginLeft: 10 }}>
        <CsvCreator filename="productcategories" headers={standart_columns} rows={tableData}>
          <p>Export</p>
        </CsvCreator>
      </Button>
      <Table loading={loading} dataSource={items} columns={manager_columns} />
      {modalInfo.type === 'POST' && (
        <Modal visible={modalInfo.visible} title="Ürün Kategori Ekleme Ekranı" onCancel={hideModal} footer={null}>
          <ProductCategoryForm handleClose={hideModal} />
        </Modal>
      )}

      {modalInfo.type === 'UPDATE' && (
        <Modal visible={modalInfo.visible} title="Ürün Kategori Güncelleme Ekranı" onCancel={hideModal} footer={null}>
